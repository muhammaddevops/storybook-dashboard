...
function FieldInput({ uid, value, onChangeHandler }) {
  return <input id={uid} className="form-control" value={value || ""} onChange={onChangeHandler} />;
}
...
function MetaFormFieldDropdown({ pushHandler, header, fields, setFields, index, hide, trackerId, refId }) {
  if (hide) {
    return null;
  }
  let links = [];
  ...
  return (
    <div className="form-group">
      ...
      <div onBlur={saveOnBlur ? onSaveHandler : null}>
        <FieldInputComponent uid={uid} field={field} value={value} onChangeHandler={onChangeHandler} />
      </div>
      {description ? (
        <small>
...
export function MetaSaveButton({ hide, pending, onSaveHandler, trackerId, refId }) {
  ...
  let pushBackButton =
    trackerId && refId && saved ? (
      <button className="btn btn-sm btn-outline-warning float-right ml-2" onClick={e => pushMetaBack(trackerId, refId)}>
...
export function SetMetaDataFields({
  ...
  const [fields, setFields] = useState(metaJson.getInPath("input_details.fields") || Traec.Im.List());
