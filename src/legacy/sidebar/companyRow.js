...
function CompanyProjectList(props) {
  ...
  let projectIds = Traec.Im.Set(
    projectList.map((i) => i?.get("uid")).filter((i) => i)
  );
  return projectList
    .filter((project) => project)
    .filter((project) =>
      projectIds.has(currentIds.projectId)
        ? currentIds.projectId == project.get("uid")
        : true
    )
    .sortBy((project) => project.get("name"))
    .map((project, i) => (
      <ProjectRow key={i} {...props} project={project} depth={depth + 1} />
    ));
}

function SubCompanyList(props) {
  let { company, companyList, companyPathIds, currentIds, depth, tenant_meta } =
    props;
  ...
  return childIds
    .filter((id) =>
      isAncestor ? companyPathIds?.has(id) || currentIds.companyId == id : true
    )
    ...
    .map((child, i) => (
      <CompanyTreeRow
        ...
        recurse={
          !(
            companyPathIds?.has(child.get("uid")) ||
            currentIds.companyId == child.get("uid")
          )
        }
      />
    ));
...
function AddCompanyWithMetaForm({ company, tenant_meta, modalId }) {
  ...
  const _setMeta = (meta) => {
    ...
    if (meta.get("Scheme Reference") && meta.get("Project Name")) {
      companyFields.name.value = `${meta.get("Scheme Reference")} - ${meta.get(
        "Project Name"
      )}`;
    }
    setMeta(meta);
    ...
    <MiniErrorBoundary>
      <BSBtnDropdown
        ...
        links={[
          ...
          {
            name: `${getTerm("Company", props)} info`,
            onClick: (e) => editMetaData(props),
          },
          {},
          {
            name: "Delete",
            onClick: (e) => deleteCompany(e, companyId, company),
          },
        ]}
      />
    </MiniErrorBoundary>
...
export default class CompanyTreeRow extends React.Component {
  constructor(props) {
    ...
    this.state = {
      collapsed: companyId
        ? localStorage.getItem(`sidebar-${companyId}`, "false") == "true"
        : false,
      formParams: {},
  ...
  render() {
    ...
    let currentId =
      currentIds.refId || currentIds.projectId || currentIds.companyId;
    ...
    let hasChildren =
      (company.get("childids") || Traec.Im.List()).size > 0 ||
      (company.get("projects") || Traec.Im.List()).size > 0;
