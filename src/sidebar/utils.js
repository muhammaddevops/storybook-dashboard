...
export const getTerm = (term, props) => {
  ...
  let term_map =
    (tenant_meta || Traec.Im.Map()).get("term_map") || Traec.Im.Map();
...
export function setNewWorkPackageFields(
  fields,
  disciplines,
  projectReportingPeriods
) {
  let setFields = Object.assign({}, fields);
  setFields.latest_commit__discipline.options =
    getDisciplineOptions(disciplines);
  setFields.latest_commit__reporting_period.options = getReportingPeriodOptions(
    projectReportingPeriods
  );
  return setFields;
}
...
export function nestDunderKeys(data) {
  ...
  for (let [key, value] of Object.entries(data)) {
    ...
    nestedData[_key] = _nest_key
      ? nestDunderKeys({ ...nestedData[_key], [_nest_key]: value })
      : value;
  }
  return nestedData;
}
